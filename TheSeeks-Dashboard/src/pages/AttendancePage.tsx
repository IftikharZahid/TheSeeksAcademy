...
import { writeStudentAttendance, setAdminDb } from '../store/slices/attendanceSlice.ts';
...
export default function AttendancePage() {
    ...
    const adminDb: Record<string, Record<string, string>> =
        useAppSelector((s: any) => s.attendance?.adminDb) ?? {};
        ...
        const unsub = onSnapshot(
            ...
            snap => {
                ...
                snap.docs.forEach(doc => {
                    const dr = doc.data().dailyRecords ?? {};
            ...
            err => { console.error('Attendance listener error:', err); setLoading(false); }
        );
        return () => unsub();
    }, [dispatch]);
    ...
    const filteredStudents = useMemo(() => {
        ...
        return list;
    }, [students, filterClass, search, filterStatus, adminDb, selectedDate]);
    ...
    useEffect(() => { setVisibleCount(PAGE_SIZE); }, [search, filterClass, filterStatus]);
    ...
    }, [loading, filteredStudents]);  // eslint-disable-line react-hooks/exhaustive-deps
                                        ...
                                        >
                                            ...
                                            <td style={{ ...TD, textAlign:'center' }}>
                                                <div style={{ display:'flex', gap:4, justifyContent:'center' }}>
                                                    {(['present','late','absent'] as StatusType[]).map(action => {
